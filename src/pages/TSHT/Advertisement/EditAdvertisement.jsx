...
const AddAdvertisement = () => {
  document.title = "Sửa Quảng Cáo | Toà Soạn Hội Tụ";
  ...
  const validation = useFormik({
    ...
    initialValues: {
      ...
      page_type: (valuePageType && valuePageType) || "",
    ...
    onSubmit: (values) => {
      const newAds = {
        ...
        display_position: valueDisplay.value,
      ...
      // save new ADS
      getAPIPutAds(newAds).then((r) => {
        if (r.status >= 0) {
          ToastCustom("Sửa quảng cáo thành công", "success");
  ...
  const getChannel = async () => {
    const res = await getAPIAdsById(id);
    setAds(res.data);
    setAvatar(res.data.image_url);
    setToDate(res.data.datetime_to);
    setFromDate(res.data.datetime_from);
    setValuePageType(res.data.page_type);
    setValueDisplay(res.data.display_position);
    onChangeDisplayType(res.data.display_position);
  };
  useEffect(() => {
    getChannel().then((r) => {});
  }, []);
